...
def verify_api_key(api_key_value: str, stored_hash: str) -> bool:
    """Verify an API key against a stored hash.

    Supports both old SHA256 hashes (64 hex characters) and new pbkdf2_sha256 hashes
    for backward compatibility. Old SHA256 hashes are automatically migrated to
    pbkdf2_sha256 when the key is successfully verified.

    Args:
        api_key_value: The plain API key value to verify
        stored_hash: The stored hash (either SHA256 hex or pbkdf2_sha256)
    ...
    try:
        if pwd_context.verify(api_key_value, stored_hash):
            return True
    except (ValueError, TypeError):
        # Hash format doesn't match pbkdf2_sha256, try old SHA256 format
        pass

    # Check if it's an old SHA256 hash (64 hex characters)
    if len(stored_hash) == 64 and all(c in '0123456789abcdefABCDEF' for c in stored_hash):
        # Compute SHA256 hash of the provided key
        computed_hash = hashlib.sha256(api_key_value.encode('utf-8')).hexdigest()
        # Compare in constant time to avoid timing attacks
        return secrets.compare_digest(computed_hash.lower(), stored_hash.lower())
...
async def get_api_key(
    ...
    for api_key in api_keys:
        ...
        if verify_api_key(api_key_value, api_key.key_hash):
            # Check expiration (use UTC for consistency with JWT tokens)
            if api_key.expires_at and api_key.expires_at < datetime.utcnow():
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    detail="API key has expired",
                )
            # Migrate old SHA256 hash to pbkdf2_sha256 if needed
            # Old hashes are 64 hex characters, new pbkdf2_sha256 hashes are longer
            if len(api_key.key_hash) == 64 and all(c in '0123456789abcdefABCDEF' for c in api_key.key_hash):
                # This is an old SHA256 hash, migrate to pbkdf2_sha256
                api_key.key_hash = get_password_hash(api_key_value)
            # Update last_used timestamp (use UTC for consistency)
